...
export function presentationsReducer(
  state: PresentationsState = presentationsInitialState,
  action: fromPresentations.Actions | fromAuthentication.Actions | RouterNavigationAction | fromSlides.Actions
): PresentationsState {
  switch (action.type) {
    ...
    case fromPresentations.SEARCH_SUCCESS: {
      return presentationsAdapter.addMany(action.payload.presentations, {
        ...state,
        count: action.payload.presentationCount,
        loading: false,
        loaded: true
      });
    }
    ...
    case fromSlides.ADD_SUCCESS: {
      ...
      return presentationsAdapter.updateOne(
        { id: presentationId, changes: { slideIds: slideIds.concat(action.payload._id) } },
        state
      );
    }
    case fromSlides.DELETE_SUCCESS: {
      ...
      const slideIds = state.entities[presentationId].slideIds.slice();
      return presentationsAdapter.updateOne(
        { id: presentationId, changes: { slideIds: slideIds.filter(slideId => slideId !== action.payload.slide._id) } },
        state
      );
    }
