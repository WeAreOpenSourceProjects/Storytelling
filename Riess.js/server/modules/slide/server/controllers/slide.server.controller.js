...
exports.create = function (req, res) {
  ...
  const presentationP = Presentation.findOne({
    _id: req.body.presentationId
  });
  Promise.all([slideP, presentationP])
    .then(function (result) {
      ...
      return Promise.all([Slide.findByIdAndUpdate(slide._id, slide, {
        new: true
      }), Presentation.findByIdAndUpdate(presentation.id, presentation)]);
    })
    .then(function (result) {
...
exports.delete = function (req, res) {
  ...
  Slide.findById(slideId)
    ...
    .then(function (slide) {
      ...
      var boxIds = boxes.map(function (box) {
        return box._id;
      })
      var images = [].concat.apply([], boxes.map(function (box) {
        return box.content.imageId || '';
      }));
    ...
    .then(function (presentation) {
      ...
      const updateSlides = presentation.slideIds
        ...
        .map(function (slide) {
          ...
          return Slide.findByIdAndUpdate(slide._id, slide.toObject(), {
            new: true
          });
        });
